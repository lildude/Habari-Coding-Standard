<?php
/**
 * Modified copy of PEAR_Sniffs_NamingConventions_ValidFunctionNameSniff.
 *
 * PHP version 5
 *
 * @category  PHP
 ...
 * @link      http://pear.php.net/package/PHP_CodeSniffer
 */
class HabariCodingStandard_Sniffs_NamingConventions_FunctionNameSniff extends PHP_CodeSniffer_Standards_AbstractScopeSniff
{
    ...
    protected function processTokenWithinScope(PHP_CodeSniffer_File $phpcsFile, $stackPtr, $currScope)
    {
		...
		/* CNS: Habari allows __ method names
...
*/
		if (preg_match('|[A-Z]|', $testMethodName) !== 0) {
			/*if ($scopeSpecified === true) {
				$error = '"%s"  must be all lowercase with words separated by underscores. Do not use camelCaseNames.';
				$data  = array(
                          ucfirst($scope),
                          $errorData[0],
                         );
				$phpcsFile->addError($error, $stackPtr, 'NotAllLowerCase', $data);
			} else { */
				$error = '"%s" must be all lowercase with words separated by underscores. Do not use camelCaseNames.';
				$phpcsFile->addError($error, $stackPtr, 'NotAllLowerCase', $errorData);
			//}
    ...
    protected function processTokenOutsideScope(PHP_CodeSniffer_File $phpcsFile, $stackPtr)
    {
		...
		// If it has capitals, it's not valid
		if (preg_match('|[A-Z]|', $functionName) !== 0) {
            $error = '"%s" must be all lowercase with words separated by underscores. Do not use camelCaseNames.';
